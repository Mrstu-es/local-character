...
export default function RoleplayApp() {
  ...
  const dedicatedChatId = view === "chats" ? activeChat : null;

  return <div className={`app-shell ${collapsed ? "sidebar-collapsed" : ""} ${dedicatedChatId ? "chat-active" : ""}`}>
    ...
    <main className={`main-content ${dedicatedChatId ? "dedicated-chat-main" : ""}`}>
      {!dedicatedChatId && <header className="topbar"><div><span className="eyebrow">ROLEPLAY CON PERSONAJES</span><h1>{navItems.find((item) => item.id === view)?.label}</h1></div><div className="topbar-actions"><span className="release-pill">0.3.0</span><button className="icon-button" onClick={() => void refresh()} disabled={busy} aria-label="Actualizar"><RefreshCw size={17} className={busy ? "spin" : ""} /></button></div></header>}
      {error && <div className="error-banner"><CircleAlert size={17} /><span>{error}</span><button onClick={() => setError(null)}>Cerrar</button></div>}
      {dedicatedChatId ? <RoleplayChatScreenModern chatId={dedicatedChatId} characters={characters} conversations={conversations} groups={groups} providers={providers} engine={engine} onBack={() => setActiveChat(null)} onOpenCharacters={() => setView("characters")} /> : <>
        ...
        {view === "chats" && <ChatsViewWithDelete characters={characters} conversations={conversations} groups={groups} providers={providers} engine={engine} activeChat={null} setActiveChat={setActiveChat} onDeleteConversation={removeConversation} onCreateGroup={createGroup} onDeleteGroup={async (id) => { if (isTauriRuntime()) { await deleteGroup(id); await deleteConversation(id); } setGroups((current) => current.filter((group) => group.id !== id)); setConversations((current) => current.filter((conversation) => conversation.id !== id)); }} onOpenCharacters={() => setView("characters")} />}
