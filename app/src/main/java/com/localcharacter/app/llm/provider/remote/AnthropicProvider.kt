...
class AnthropicProvider(
    ...
    override suspend fun getModels(): List<LlmModelInfo> {
        ...
        return (root["data"] as? JsonArray)?.mapNotNull { row ->
            val model = row.asObject() ?: return@mapNotNull null
            val id = model.string("id") ?: return@mapNotNull null
            LlmModelInfo(providerId, id, model.string("display_name") ?: id, PricingType.PAID, supportsReasoning = true)
        }.orEmpty()
    }
    ...
    override fun generate(request: LlmRequest): Flow<LlmStreamEvent> = streamSse(
        buildRequest = { key ->
            val payload = buildJsonObject {
                ...
                if (request.stopSequences.isNotEmpty()) put("stop_sequences", buildJsonArray {
                    request.stopSequences.forEach { add(JsonPrimitive(it)) }
                })
            }
            apiRequest("${definition.baseUrl}/messages", key)
    ...
    private fun parseEvent(event: SseEvent): List<LlmStreamEvent> {
        ...
        return when (root.string("type") ?: event.event) {
            "content_block_delta" -> root.obj("delta")?.takeIf { it.string("type") == "text_delta" }
                ?.string("text")?.let { listOf(LlmStreamEvent.TextDelta(it)) }.orEmpty()
            "message_start" -> root.obj("message")?.obj("usage")?.let {
                listOf(LlmStreamEvent.Usage(TokenUsage(inputTokens = it.long("input_tokens"), outputTokens = it.long("output_tokens"))))
            }.orEmpty()
            "message_delta" -> root.obj("usage")?.let {
                listOf(LlmStreamEvent.Usage(TokenUsage(outputTokens = it.long("output_tokens"))))
            }.orEmpty()
            "message_stop" -> listOf(LlmStreamEvent.Completed)
            "error" -> listOf(LlmStreamEvent.Error(ProviderErrorMapper.http(500, root.toString())))
            else -> emptyList()
        }
    }
